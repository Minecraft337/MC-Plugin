...
import com.mcplugin.features.magnet.MagnetManager.MagnetCluster;
...
/**
 * Отвечает за сохранение и загрузку магнитных кластеров из SQLite.
 */
public class MagnetPersistence {
    ...
    public static void saveCluster(MagnetCluster cluster) {
        ...
        try {
            ...
            PreparedStatement del = con.prepareStatement("DELETE FROM magnet_blocks WHERE magnet_id = ?");
            del.setInt(1, cluster.id);
            del.executeUpdate();
            del.close();

            PreparedStatement ins = con.prepareStatement("""
                INSERT INTO magnet_blocks (magnet_id, x, y, z) VALUES (?, ?, ?, ?)
            """);
    ...
    public static void deleteCluster(int id) {
        Connection con = DatabaseManager.getConnection();
        try {
            PreparedStatement delBlocks = con.prepareStatement("DELETE FROM magnet_blocks WHERE magnet_id = ?");
            delBlocks.setInt(1, id);
            delBlocks.executeUpdate();
            delBlocks.close();

            PreparedStatement delMagnet = con.prepareStatement("DELETE FROM magnets WHERE id = ?");
    ...
    public static int loadAll(Map<Long, MagnetCluster> locationToCluster,
                              Map<Integer, MagnetCluster> clustersById,
                              int nextIdStart) {
        ...
        int maxId = 0;
        try {
            PreparedStatement ps = con.prepareStatement("SELECT * FROM magnets WHERE active = 1");
            ...
            while (rs.next()) {
                ...
                cluster.id = id;
                cluster.world = world;
                cluster.center = new Location(world,
                        rs.getInt("center_x"),
                        rs.getInt("center_y"),
                        rs.getInt("center_z"));
